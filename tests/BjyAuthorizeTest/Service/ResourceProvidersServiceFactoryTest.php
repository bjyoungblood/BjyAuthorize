 ...
 */
class ResourceProvidersServiceFactoryTest extends PHPUnit_Framework_TestCase
{
    /**
    ...
    {
        $factory        = new ResourceProvidersServiceFactory();
        ...
        $config         = array(
            'resource_providers' => array(
                'foo'                         => array(),
                'bar'                         => array(),
                __NAMESPACE__ . '\\MockResourceProvider' => array('option' => 'value'),
            ),
        );
        ...
        $invokableProvider = array_filter(
            $providers,
            function ($item) {
                return $item instanceof MockResourceProvider;
            }
        );

        $this->assertCount(1, $invokableProvider);

        /* @var $invokableGuard \BjyAuthorizeTest\Service\MockResourceProvider */
        $invokableProvider = array_shift($invokableProvider);

        $this->assertInstanceOf(__NAMESPACE__ . '\\MockResourceProvider', $invokableProvider);
